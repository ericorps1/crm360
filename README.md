...
CRM360 es un CRM self-hosted y gratuito para negocios que venden por WhatsApp:
...
![Bandeja de CRM360 CRM](docs/screenshots/bandeja.png)
...
```bash
git clone https://github.com/kevinrivm/crm360.git vocero && cd vocero
...
CRM360 **consume** un token de la WhatsApp Cloud API — no implementa el
...
3. En CRM360: **Configuración → WhatsApp** → pega WABA ID + Phone Number ID +
   token → **Probar conexión** → Guardar.
4. En el panel de Meta (WhatsApp → Configuration → Webhook) pega la **URL del
   webhook** y el **verify token** que CRM360 te muestra, y suscribe el campo
...
clientes; la instancia de CRM360 del cliente solo recibe su token. El webhook
...
```text
   Meta (WABA del cliente)
        ...
        ▼
   ┌────────────────────────────┐      ┌─────────────────────────────┐
   │  Instancia CRM360          │      │  Backend de TU agencia      │
...
> la app dueña. Por eso CRM360 también **sincroniza plantillas por la API de
...
1. **Opt-in**: escribe solo a personas que iniciaron la conversación o
   aceptaron recibir mensajes; CRM360 respeta la ventana de 24 h y bloquea el
...
4. **Sin spam ni broadcast**: CRM360 no incluye envíos masivos; úsalo para
...
permitidos (números de prueba de Meta) o el formato es inválido. CRM360 ya
...
CRM360 es un derivado de [Vocero CRM](https://github.com/kevinrivm/vocero-crm),
de Kevin Belier, publicado bajo licencia MIT. Se conserva la licencia original
en `LICENSE`.

Cambios propios respecto al original:

- Identidad fija **CRM360** (nombre y colores no configurables)
- Paleta violeta primario + turquesa secundario, con tokens para claro y oscuro
- Modo oscuro en negro por capas de elevación, con interruptor claro/oscuro/sistema
- Badges rectangulares de estatus en el embudo, con color atado al avance
- Avatares neutros (gris en claro, negro en oscuro)
- "Pipeline" renombrado a "Embudo" en toda la interfaz
- Credenciales de demo precargadas solo en desarrollo
