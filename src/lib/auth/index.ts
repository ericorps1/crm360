...
const globalForSignup = globalThis as unknown as {
  __crm360InternalSignup?: AsyncLocalStorage<boolean>;
};
...
function internalSignupContext(): AsyncLocalStorage<boolean> {
  if (!globalForSignup.__crm360InternalSignup) {
    globalForSignup.__crm360InternalSignup = new AsyncLocalStorage<boolean>();
  }
  return globalForSignup.__crm360InternalSignup;
}
...
const globalForAuth = globalThis as unknown as { __crm360Auth?: Auth };

export function getAuth(): Auth {
  if (!globalForAuth.__crm360Auth) globalForAuth.__crm360Auth = createAuth();
  return globalForAuth.__crm360Auth;
}
