...
export async function listConversations(
  ...
  // El tono viaja junto al nombre: así el punto de la bandeja usa el mismo
  // color que la columna del kanban, en vez de un mapa por nombre que se
  // rompe en cuanto alguien renombra una etapa.
  const stageToneSql = sql<string | null>`(
    select s.tone from lead l
    join pipeline_stage s on s.id = l.stage_id
    where l.contact_id = ${schema.contact.id}
    limit 1
  )`;
