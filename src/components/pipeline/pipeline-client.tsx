        ...
        >
          ...
          <DragOverlay>
            {activeLead ? (
              <LeadCard
                lead={activeLead}
                // La tarjeta arrastrada conserva el color de su etapa de origen.
                tone={stages.find((s) => s.id === activeLead.stageId)?.tone}
                overlay
              />
            ) : null}
          </DragOverlay>
...
function StageColumn({ stage, leads }: { stage: StageDto; leads: BoardLead[] }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "flex h-full w-64 shrink-0 flex-col rounded-lg border bg-subtle transition-shadow",
        isOver && "ring-2 ring-primary/60"
      )}
    >
      <div className="flex items-center justify-between border-b px-3 py-2.5">
        <span className="flex min-w-0 items-center gap-1.5 text-sm font-semibold">
          {stage.kind === "won" && <Trophy className="h-3.5 w-3.5 text-text-3" />}
          {stage.kind === "lost" && <XCircle className="h-3.5 w-3.5 text-text-3" />}
          <span className="truncate">{stage.name}</span>
        </span>
        {/* El color vive aquí y solo aquí: un cuadrito, no toda la columna. */}
        <span className={cn("badge-etapa tabular-nums", toneClass(stage.tone))}>
          {leads.length}
        </span>
      </div>
      <div className="flex-1 space-y-2 overflow-y-auto p-2">
        {leads.map((lead) => (
          <DraggableLead key={lead.id} lead={lead} tone={stage.tone} />
        ))}
        {leads.length === 0 && (
          <p className="px-2 py-6 text-center text-xs text-text-4">Sin tarjetas</p>
        )}
      </div>
    </div>
  );
...
function DraggableLead({ lead, tone }: { lead: BoardLead; tone: string | null }) {
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <p className="flex items-center gap-1.5 truncate text-sm font-medium">
            {tone && <span className="cuadro-etapa shrink-0" aria-hidden />}
