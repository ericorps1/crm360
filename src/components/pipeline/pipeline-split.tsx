...
/**
 * Vista de conversación del embudo: una etapa a la vez, con el hilo integrado.
 * Es la vista secundaria; el tablero kanban (PipelineBoard) es la principal.
 */
export function PipelineSplit({ viewSwitcher }: { viewSwitcher?: ReactNode }) {
  ...
  // Llegada con ?contact= (desde el tablero o un enlace): abre esa
  // conversación una sola vez, igual que hace la bandeja.
  const contactParam = searchParams.get("contact");
  const contactAplicado = useRef(false);
  useEffect(() => {
    if (!contactParam || contactAplicado.current || selectedId) return;
    const conv = byContactId.get(contactParam);
    if (conv) {
      contactAplicado.current = true;
      select(conv.id);
    }
  }, [contactParam, byContactId, selectedId, select]);
